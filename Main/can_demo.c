...
#if 1		//DES 1=uart init, 0=default code
...
#if 1		//DES 1=init UART, 0=default code
  int received_data=NULL;
  int result = BSP_Init(BSP_INIT_BCC);
  printf("\n\rDid this work????\r\n");
  printf("hope so! result = 0x%04X\r\n", result);
  printf("Please enter one number for me to add to 4:   ");
#if 1		//DES 1=test, 0=default code
  do {
	   received_data = RETARGET_ReadChar();
  }while(received_data == -1);
  printf("\r\nreceived_data = %c", (char)received_data);
#else
  scanf("%d", &num_entered);
#endif
  printf("\r\nNumber entered was %d\r\n %d+4=%d\r\n", received_data, received_data, (received_data+4));
#endif
